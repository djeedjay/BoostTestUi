...
struct DteInstance
{
	DteInstance(const std::wstring& description, EnvDTE::_DTE* pIDte);

	std::wstring description;
	CComPtr<EnvDTE::_DTE> pIDte;
};
...
{
public:
	explicit SelectDevEnvDlg(const std::vector<DteInstance>& instances, int selection = -1);
...
private:
	...
	const std::vector<DteInstance>& m_instances;
	int m_index;
};
